...
public class SculkGrinderBlock extends Block {
    ...
    public @NotNull ItemInteractionResult useItemOn(ItemStack stack, BlockState state, Level level, BlockPos pos, Player player, InteractionHand hand, BlockHitResult hitResult) {
        ...
        if (stack.is(JNETags.Items.SCULK_GRINDABLES)) {
            ...
            if (level instanceof ServerLevel serverLevel) ExperienceOrb.award(serverLevel, hitResult.getLocation(), JNEConfigs.SCULK_GRINDER_EXPERIENCE.get());
            if (!player.getAbilities().instabuild) stack.shrink(1);
            blockParticle(level, pos, JNEParticleTypes.TREACHEROUS_FLAME.get());
            bl = true;
        } else if (stack.is(Items.SCULK)) {
            level.playSound(player, pos, SoundEvents.SCULK_BLOCK_SPREAD, SoundSource.BLOCKS, 1.0f, level.getRandom().nextFloat() * 0.4f + 0.8f);
            if (!level.isClientSide()) Block.popResourceFromFace(level, pos, hitResult.getDirection(), new ItemStack(JNEBlocks.SCULK_GRINDER.get()));
            if (!player.getAbilities().instabuild) stack.shrink(1);
            blockParticle(level, pos, ParticleTypes.SCULK_CHARGE_POP);
            bl = true;
        }

        if (bl) return ItemInteractionResult.sidedSuccess(level.isClientSide);
        return ItemInteractionResult.PASS_TO_DEFAULT_BLOCK_INTERACTION;
    }

    @Override
    public void animateTick(BlockState state, Level level, BlockPos pos, RandomSource random) {
        if (random.nextInt(2) == 0) blockParticle(level, pos, ParticleTypes.SCULK_CHARGE_POP);
    }
