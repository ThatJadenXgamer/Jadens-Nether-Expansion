...
public class SuspiciousSoulSandBlockRenderer implements BlockEntityRenderer<SuspiciousSoulSandBlockEntity> {
    private final ItemRenderer itemRenderer;

    public SuspiciousSoulSandBlockRenderer(BlockEntityRendererProvider.Context context) {
        this.itemRenderer = context.getItemRenderer();
    }

    // Taken from Vanilla's BrushableBlockRenderer code
    @Override
    public void render(SuspiciousSoulSandBlockEntity blockEntity, float tickDelta, PoseStack poseStack, MultiBufferSource multiBufferSource, int light, int overlay) {
        if (blockEntity.getLevel() != null) {
            int k = blockEntity.getBlockState().getValue(BlockStateProperties.DUSTED);
            if (k > 0) {
                ...
                if (direction != null) {
                    ...
                    if (!itemStack.isEmpty()) {
                        ...
                        this.itemRenderer.renderStatic(itemStack, ItemDisplayContext.FIXED, l, OverlayTexture.NO_OVERLAY, poseStack, multiBufferSource, blockEntity.getLevel(), 0);
                        poseStack.popPose();
                    }
                }
            }
