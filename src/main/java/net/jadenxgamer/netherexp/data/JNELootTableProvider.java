...
public class JNELootTableProvider extends LootTableProvider {
    ...
    private record ArchaeologyLoot(HolderLookup.Provider registries) implements LootTableSubProvider {
        ...
        private LootPoolSingletonContainer.Builder<?> item(ItemLike item, int weight, float count) {
            return LootItem.lootTableItem(item)
                    .setWeight(weight)
                    .apply(SetItemCountFunction.setCount(ConstantValue.exactly(count)));
        }

        private LootPoolSingletonContainer.Builder<?> item(ItemLike item, int weight, float min, float max) {
            return LootItem.lootTableItem(item)
                    .setWeight(weight)
                    .apply(setCount(min, max));
        }

        private LootItemFunction.Builder setCount(float min, float max) {
    ...
    private static final class BlockLoot extends BlockLootSubProvider {
        ...
        protected Iterable<Block> getKnownBlocks() {
            return List.of();
            // return JNEBlocks.BLOCKS.getEntries().stream().map(e -> (Block) e.value()).toList();
        }
