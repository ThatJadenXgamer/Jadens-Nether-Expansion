...
public abstract class WeepingVinesMixin extends AbstractPlantStemBlock {
    ...
    public void randomDisplayTick(BlockState state, World world, BlockPos pos, Random random) {
        ...
        if (!budding && f < 0.3) {
            world.addParticle(JNEParticles.FALLING_SHROOMLIGHT, x, y, z, 0.0, 0.0, 0.0);
        }
    }
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ...
        boolean bl = false;
        if (budding) {
            if (itemStack.isOf(JNEItems.LIGHTSPORES)) {
    ...
    private static void dropLight(World world, BlockPos pos) {
        dropStack(world, pos, new ItemStack(JNEItems.LIGHTSPORES, 1));
    }
