...
extends AbstractFireBlock {
    ...
    public void randomDisplayTick(BlockState state, World world, BlockPos pos, Random random) {
        if (NetherExp.getConfig().visualeffects.improved_soul_fire_particles) {
            ...
            world.addParticle(JNEParticles.SOUL_EMBER, (double)pos.getX() + 0.5 + random.nextDouble() / 4.0 * (double)(random.nextBoolean() ? 1 : -1), (double)pos.getY() + 0.6, (double)pos.getZ() + 0.5 + random.nextDouble() / 4.0 * (double)(random.nextBoolean() ? 1 : -1), 0.0, 0.015, 0.0);
        }
